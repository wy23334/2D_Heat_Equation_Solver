// Phase 3 negative-control baseline: deliberately traverse a row-major array
// by columns to demonstrate the cost of poor spatial locality.
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
...
int main(int argc, char **argv) {
    ...
    for (iter = 1; iter <= max_iters; iter++) {
        ...
        // Deliberately poor order for C row-major storage: adjacent accesses
        // are n doubles apart. This file is a Phase 3 teaching baseline only.
        for (int j = 1; j < n - 1; j++) {
            for (int i = 1; i < n - 1; i++) {
